/**
 *  SourceUnit: \\wsl.localhost\Ubuntu\root\workspaceWSL\cdk-validium-contracts\contracts\CDKValidium.sol
 */
...
interface ICDKDataCommittee {
    function verifySignatures(
        bytes32 hash,
        bytes memory signaturesAndAddrs
    ) external view;
}
    ...
    ICDKValidiumErrors
{
    ...
    IERC20Upgradeable public immutable matic =
        IERC20Upgradeable(0xED1Ff62dc080B12eCcb19Ff09D6827BABD498D91);

    // Rollup verifier interface
    IVerifierRollup public immutable rollupVerifier =
        IVerifierRollup(0x985E89135Fb4968bca0DB46422dc1Ed31b985D52);

    // Global Exit Root interface
    IPolygonZkEVMGlobalExitRoot public immutable globalExitRootManager =
        IPolygonZkEVMGlobalExitRoot(0x0F662cEbC4be7a4d7438a6215bC05C6Feb2F49D1);

    // PolygonZkEVM Bridge Address
    IPolygonZkEVMBridge public immutable bridgeAddress =
        IPolygonZkEVMBridge(0x44db454C15846c5521d4C9581b31B5C359518C73);

    // CDK Data Committee Address
    ICDKDataCommittee public immutable dataCommitteeAddress =
        ICDKDataCommittee(0xdd9eb731A15DB512093925499A93ae9Ba75Ac972);

    // L2 chain identifier
    uint64 public immutable chainID = 28518;

    // L2 chain identifier
    uint64 public immutable forkID = 6;
    ...
    function sequenceBatches(
        ...
        dataCommitteeAddress.verifySignatures(
            currentAccInputHash,
            signaturesAndAddrs
        );
...
contract CDKValidiumV2 is CDKValidiumBase {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    IVerifierRollup public immutable rollupVerifierV2;

    constructor(IVerifierRollup _rollupVerifier2) {
        rollupVerifierV2 = _rollupVerifier2;
    }
    ...
    function _verifyAndRewardBatches(
        ...
        uint256 inputSnark = uint256(sha256(snarkHashBytes)) % _RFIELD;
        // Verify proof
        if (!rollupVerifierV2.verifyProof(proof, [inputSnark])) {
            revert InvalidProof();
        }
