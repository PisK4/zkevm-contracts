...
async function main() {
    ...
    // TODO: replcace zkEVMDeployerAddress to CDKValidiumDeployer address
    ...
    if (proxyAdminOwner !== deployer.address) {
        throw new Error(
            `Proxy admin was deployed, but the owner ${proxyAdminOwner} is not the deployer, deployer address: ${deployer.address}, proxyAdmin: ${proxyAdminOwner}`
        );
    }
    ...
    const timelockAddressRollupManager = "";
    const polygonZkEVMGlobalExitRoot = "";
            ...
            try {
                polygonRollupManagerContract = await upgrades.deployProxy(
                    ...
                    ],
                    {
                        ...
                        constructorArgs: [polygonZkEVMGlobalExitRoot, polTokenAddress, polygonZkEVMBridgeContract],
                        unsafeAllow: ["constructor", "state-variable-immutable"],
                    }
                );
